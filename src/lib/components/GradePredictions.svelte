    ...
    {#if gradePredictions.size > 0}
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {#each Array.from(gradePredictions.entries()) as [subject, prediction]}
          <div class="p-3 rounded-lg bg-white/80 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 relative group transition-all duration-200 hover:scale-[1.02] cursor-pointer">
            <div class="flex justify-between items-center">
              <span class="text-sm font-medium text-zinc-900 dark:text-white truncate">{subject}</span>
            ...
            <div class="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-full mb-2 px-4 py-3 bg-zinc-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-20 max-w-xs w-max whitespace-pre-line break-words shadow-lg border border-zinc-700"
              ...
              <div class="absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-6 border-r-6 border-t-6 border-transparent border-t-zinc-900"></div>
            </div>
          </div>
        {/each}
      </div>
