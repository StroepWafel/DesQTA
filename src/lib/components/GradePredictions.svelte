...
{#if aiIntegrationsEnabled && gradeAnalyserEnabled && currentView === showInView}
  {#if aiIntegrationsEnabled && gradeAnalyserEnabled}
    <div
      ...
      {#if predictionError}
        <div
          class="p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {#each Array.from(gradePredictions.entries()) as [subject, prediction]}
            <div
              ...
              <div
                class="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-full mb-2 px-4 py-3 bg-zinc-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-20 max-w-xs w-max whitespace-pre-line wrap-break-word shadow-lg border border-zinc-700"
                style="min-width:220px; max-width:320px;">
                <div class="font-semibold mb-1">AI Reasoning:</div>
                <div style="white-space:pre-line; word-break:break-word;">
                  {getPredictedGradeDisplay(subject)?.reasoning}
                </div>
                <div class="mt-1 text-zinc-300">
                  Confidence: {getPredictedGradeDisplay(subject)?.confidence}%
                </div>
                <div
                  class="absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-6 border-r-6 border-t-6 border-transparent border-t-zinc-900">
                </div>
              </div>
            </div>
          {/each}
