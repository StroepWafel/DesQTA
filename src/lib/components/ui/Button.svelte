<script lang="ts">
  ...
  const variants = {
    primary:
      'bg-accent-500 text-white hover:bg-accent-600 focus:ring-accent-500 border-transparent',
    secondary:
      'bg-zinc-100 text-zinc-700 hover:bg-zinc-200/50 dark:bg-zinc-700/50 dark:text-zinc-300 dark:hover:bg-zinc-600 focus:ring-zinc-500 border-transparent',
    ghost:
      'bg-transparent text-zinc-700 hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-700 focus:ring-zinc-500 border-transparent',
  ...
  let baseClasses = $derived(
    [
      'inline-flex items-center justify-center gap-2 font-medium rounded-lg border transition-all duration-200 ease-in-out transform',
...
<button {type} class={baseClasses} {disabled} {onclick} aria-label={ariaLabel} aria-busy={loading}>
  {#if loading}
