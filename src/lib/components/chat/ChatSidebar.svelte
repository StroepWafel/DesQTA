...
<aside class="w-64 min-w-[200px] max-w-xs bg-white/10 dark:bg-zinc-900/60 border-r border-zinc-300/50 dark:border-zinc-800/50 shadow-md rounded-xl m-2 p-4 flex flex-col gap-4 backdrop-blur-xs">
  {#if cloudUser}
    <div class="flex items-center gap-3 mb-4">
      ...
      {/if}
      <div>
        <div class="font-semibold text-base text-zinc-900 dark:text-white">{cloudUser.displayName || cloudUser.username}</div>
        <div class="text-xs text-zinc-500 dark:text-zinc-300">{cloudUser.email}</div>
      </div>
    </div>
  {/if}
  
  <div class="flex items-center justify-between mb-2">
    <h2 class="font-semibold text-lg text-zinc-900 dark:text-white">Conversations</h2>
    <button 
  ...
  <div class="flex-1 overflow-y-auto">
    <!-- Groups Section -->
    {#if groupsLoading}
      <div class="text-zinc-500 dark:text-zinc-300 text-sm mb-4">Loading groups...</div>
    {:else if groupsError}
      <div class="text-red-500 dark:text-red-400 text-sm mb-4">{groupsError}</div>
    {:else if groups.length > 0}
      <div class="mb-6">
        <h3 class="text-sm font-medium text-zinc-600 dark:text-zinc-400 mb-3 px-2">Groups</h3>
        <div class="space-y-1">
          {#each groups as group}
            <button 
              class="flex items-center gap-3 w-full px-3 py-3 rounded-lg transition-all duration-200 hover:bg-accent-100/30 dark:hover:bg-accent-700/30 hover:scale-[1.02] focus:outline-hidden focus:ring-2 ring-accent-500 {selectedGroup?.id === group.id ? 'bg-accent-500/20 text-accent-500' : 'text-zinc-900 dark:text-white'}" 
            ...
            >
              ...
              <div class="flex-1 text-left">
                ...
                <div class="text-xs text-zinc-500 dark:text-zinc-400">Group</div>
              </div>
            </button>
          {/each}
        </div>
    ...
    {#if friendsLoading}
      <div class="text-zinc-500 dark:text-zinc-300 text-sm">Loading friends...</div>
    {:else if friendsError}
      <div class="text-red-500 dark:text-red-400 text-sm">{friendsError}</div>
    {:else if friends.length === 0}
      <div class="text-zinc-500 dark:text-zinc-300 text-sm">No friends found.</div>
    {:else}
      <div>
        <h3 class="text-sm font-medium text-zinc-600 dark:text-zinc-400 mb-3 px-2">Direct Messages</h3>
        <div class="space-y-1">
          {#each friends as friend}
            <button 
              class="flex items-center gap-3 w-full px-3 py-3 rounded-lg transition-all duration-200 hover:bg-accent-100/30 dark:hover:bg-accent-700/30 hover:scale-[1.02] focus:outline-hidden focus:ring-2 ring-accent-500 {selectedFriend?.id === friend.id ? 'bg-accent-500/20 text-accent-500' : 'text-zinc-900 dark:text-white'}" 
