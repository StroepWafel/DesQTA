...
<button
  class="group w-full text-left p-3 sm:p-3.5 mb-2 transition-all duration-200 flex gap-3 rounded-xl relative animate-fadeIn border
  ...
  <div class="flex-1 min-w-0 flex flex-col gap-1.5">
    ...
    <div class="flex justify-between items-center w-full">
      <div class="flex items-center gap-2 min-w-0">
        <span class="font-semibold text-[13px] sm:text-sm truncate {msg.unread ? 'text-accent-600' : 'text-zinc-900 dark:text-zinc-100'}">{msg.sender}</span>
        {#if msg.unread}
          <span class="w-1.5 h-1.5 rounded-full bg-accent-500"></span>
        {/if}
      </div>
      <span class="ml-2 shrink-0 text-[11px] sm:text-xs px-2 py-0.5 rounded-full bg-zinc-200/50 dark:bg-zinc-800/60 text-zinc-700 dark:text-zinc-300">{formatDate(msg.date)}</span>
    </div>
    ...
    <!-- Row 3: to (sm+) + preview -->
    <div class="flex items-center gap-1.5 text-[12px] sm:text-xs text-zinc-600 dark:text-zinc-400 min-w-0">
