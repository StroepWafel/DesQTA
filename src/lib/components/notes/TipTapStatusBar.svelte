<script lang="ts">
  ...
  export let editorName: string = 'TipTap';
...
<div
  ...
  <div class="flex items-center space-x-4">
    ...
    <div class="flex items-center space-x-1">
      <div
        class="w-2 h-2 rounded-full {isFocused ? 'bg-green-500' : 'bg-zinc-400 dark:bg-zinc-600'}">
      </div>
    ...
    <!-- Auto-save status -->
    {#if autoSaveEnabled}
      <span class="text-zinc-400 dark:text-zinc-500"> Auto-save enabled </span>
    {:else}
      <span class="text-zinc-400 dark:text-zinc-500"> Auto-save disabled </span>
    {/if}
