...
{#if isOpen}
  ...
  >
    <div
      class="w-full max-w-md bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-border overflow-hidden"
    ...
    >
      ...
      <div class="flex items-center justify-between p-6 border-b border-border">
        <h2 id="image-modal-title" class="text-lg font-semibold text-foreground">
          Image Options
        </h2>
        <button
          class="p-2 rounded-lg text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 hover:surface-muted transition-all duration-200"
          onclick={() => dispatch('close')}
          title="Close"
        >
      ...
      <div class="p-6 space-y-2">
        <!-- Copy Alt Text -->
        <button
          class="w-full flex items-center px-4 py-3 text-left rounded-lg text-foreground hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all duration-200 hover:scale-[1.01] focus:outline-hidden focus:ring-2 accent-ring"
      ...
      <!-- Footer -->
      <div class="px-6 py-4 border-t border-border bg-zinc-50 dark:bg-zinc-800">
        <div class="flex justify-end">
          <button
            class="px-4 py-2 text-sm rounded-lg text-muted-foreground hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-all duration-200"
            onclick={() => dispatch('close')}
          >
            Cancel
          </button>
