<script lang="ts">
  ...
  async function loadSubjectsAndColours() {
    ...
    const all: SubjectItem[] = folders
      .flatMap((f: any) => f.subjects)
      .map((s: any) => ({
        code: s.code,
        title: s.title,
    ...
    try {
      ...
      if (cached && Array.isArray(cached.assessments)) {
        upcomingAssessments = cached.assessments
          ...
          .sort(
            (a: AssessmentItem, b: AssessmentItem) =>
              new Date(`${a.due_date}T${a.due_time ?? '00:00'}:00`).getTime() -
              new Date(`${b.due_date}T${b.due_time ?? '00:00'}:00`).getTime(),
          )
          .slice(0, 5);
      } else {
  ...
  function updateField<T extends keyof TodoItem>(todoId: string, field: T, value: TodoItem[T]) {
    todos = todos.map((t) =>
      t.id === todoId
        ? ({ ...t, [field]: value, updated_at: new Date().toISOString() } as TodoItem)
        : t,
    );
    saveTodos();
  }
              ...
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <select
                    ...
                    <option value="today"
                      ><T key="study.filter_today" fallback="Due Today" /></option>
                    <option value="week"
                      ><T key="study.filter_this_week" fallback="This Week" /></option>
                    <option value="completed"
                      ><T key="study.filter_completed" fallback="Completed" /></option>
                  </select>
              ...
              <div class="mt-2 flex items-center justify-between">
                <span class="text-sm text-zinc-500 dark:text-zinc-400">
                  <T
                    ...
                    values={{ count: filteredSortedTodos.length }} />
                </span>
              </div>
            </div>
                ...
                {#each filteredSortedTodos as todo (todo.id)}
                  <div
                    class="rounded-xl border border-zinc-200/50 dark:border-zinc-700/50 bg-white/60 dark:bg-zinc-800/40 backdrop-blur-sm p-4 transition-all duration-200 hover:bg-white/80 dark:hover:bg-zinc-800/60 hover:border-zinc-300/60 dark:hover:border-zinc-600/60 hover:shadow-lg {completingTasks.has(
                      ...
                      : ''} {deletingTasks.has(todo.id) ? 'opacity-50 scale-95' : ''}"
                    in:fly={{
                      y: 20,
                      duration: 300,
                      delay: Math.min(filteredSortedTodos.indexOf(todo) * 30, 200),
                      easing: quintOut,
                    }}
                        ...
                        <div class="flex-1 min-w-0">
                          <div
                            ...
                            <div class="flex items-center gap-1 sm:gap-2 shrink-0">
                              <span
                                class="text-xs px-1.5 py-0.5 sm:px-2 rounded-full border priority-indicator {getPriorityStyles(
                                  todo.priority,
                          ...
                          <div class="mt-2 flex flex-wrap items-center gap-2 text-sm">
                            {#if todo.related_subject}
                              {#key todo.related_subject}
                                <span
                                  ...
                                  style={chipStylesForCode(
                                    getSubjectCodeFromLabel(todo.related_subject),
                                  )}>{todo.related_subject}</span>
                              {/key}
                            {/if}
                            {#if todo.related_assessment}
                              {#key todo.related_assessment}
                                <span
                                  class="px-2 py-0.5 rounded-lg border text-xs"
                                  style={chipStylesForCode(
                                    getSubjectCodeFromLabel(todo.related_subject) || null,
                                  )}>{todo.related_assessment}</span>
                              {/key}
                            {/if}
                            {#if todo.due_date || todo.due_time}
                              <span
                                class="inline-flex items-center gap-3 text-zinc-600 dark:text-zinc-300">
                                {#if todo.due_date}
                                  <span class="inline-flex items-center gap-1">
                                    <Icon src={Calendar} class="w-4 h-4" />{formatDueDate(
                                      todo.due_date,
                                    )}
                                  </span>
                                {/if}
                                {#if todo.due_time}
                                  <span class="inline-flex items-center gap-1">
                                    <Icon src={Clock} class="w-4 h-4" />{formatDueTime(
                                      todo.due_time,
                                    )}
                                  </span>
                                {/if}
                            ...
                            {#if (todo.tags ?? []).length}
                              <span
                                class="px-2 py-0.5 rounded-lg border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200"
                                >{(todo.tags ?? []).join(', ')}</span>
                            {/if}
                          </div>
                          ...
                          {#if (todo.subtasks ?? []).length}
                            <div class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
                              <T
                                ...
                                values={{
                                  completed: (todo.subtasks ?? []).filter((s) => s.completed)
                                    .length,
                                  total: todo.subtasks?.length || 0,
                                }} />
                            </div>
                          {/if}
                          <div
                            class="mt-3 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                            <Button
                              variant="ghost"
                              size="xs"
                              onclick={() => markAllSubtasksDone(todo.id)}
                              >✓ <T key="study.all_subtasks" fallback="All subtasks" /></Button>
                            <Button variant="ghost" size="xs" onclick={() => duplicateTodo(todo.id)}
                              ><T key="study.duplicate" fallback="Duplicate" /></Button>
                          </div>
                        </div>
                      </div>
                    {:else}
                        ...
                        <div class="flex-1 min-w-0">
                          <div class="flex items-center gap-2">
                            ...
                            <div class="flex flex-col gap-1">
                              ...
                              <span
                                class="text-xs px-2 py-0.5 rounded-full border priority-indicator {getPriorityStyles(
                                  todo.priority,
                                )} text-center">{todo.priority ?? 'medium'}</span>
                            </div>
                          </div>
                          <div class="mt-2 grid grid-cols-1 gap-2">
                              ...
                              {#if showSubjectDropdown[todo.id]}
                                <div
                                  ...
                                  {#each subjects.filter( (s) => (subjectQuery[todo.id] ?? todo.related_subject ?? '')
                                        .toLowerCase()
                                        .split(' ')
                                        .every((q) => s.title.toLowerCase().includes(q) || s.code
                                              .toLowerCase()
                                              .includes(q)), ) as s}
                                    <button
                                ...
                                <div
                                  ...
                                  {#each assessmentsAll
                                    .filter((a) => {
                                      ...
                                      const subjectCode = (subjectQuery[todo.id] ?? todo.related_subject ?? '')
                                          .split(' — ')[0]
                                          ?.toLowerCase();
                                      const matchesText = a.title
                                          .toLowerCase()
                                          .includes(q) || a.code.toLowerCase().includes(q);
                                    ...
                                    <button
                                      ...
                                      on:mousedown={() => {
                                        ...
                                        if (!todo.related_subject) {
                                          ...
                                          if (subj) {
                                            todo.related_subject = `${subj.code} — ${subj.title}`;
                                          }
                                        }
                                        showAssessmentDropdown[todo.id] = false;
                                      }}>
                                      <span
                                        class="mr-2 inline-block w-2 h-2 rounded-full"
                                        style="background-color: {subjectColours[a.code] ||
                                          a.colour ||
                                          '#8e8e8e'}"></span>
                            ...
                            {#if todo.due_date || todo.due_time}
                              <div
                                class="text-sm text-zinc-600 dark:text-zinc-300 inline-flex items-center gap-3">
                                {#if todo.due_date}
                                  <span class="inline-flex items-center gap-1"
                                    ><Icon src={Calendar} class="w-4 h-4" />{formatDueDate(
                                      todo.due_date,
                                    ...
                                    <div class="flex items-center gap-2">
                                      ...
                                      <input
                                        ...
                                        on:blur={() => {
                                          ...
                                          updateField(todo.id, 'subtasks', list);
                                        }}
                                        placeholder={$_('study.subtask_title') ||
                                          'Subtask title'} />
                                    </div>
                                  {/each}
                                </div>
            ...
            <div class="flex-1 min-h-0 p-6 overflow-y-auto">
              {#if loadingAssessments}
                <div class="flex items-center justify-center py-6">
                  <div
                    class="w-10 h-10 rounded-full border-4 border-zinc-300 dark:border-zinc-700 border-t-transparent animate-spin">
                  </div>
                </div>
              {:else}
                <div class="space-y-3">
                  {#each upcomingAssessments as a (a.id)}
                    <div
                      ...
                      in:fly={{
                        x: 20,
                        duration: 300,
                        delay: Math.min(upcomingAssessments.indexOf(a) * 50, 200),
                        easing: quintOut,
                      }}>
                      <!-- Left accent bar -->
                      <span
                        ...
                        style="background-color: {a.colour ||
                          subjectColours[a.subject?.split(' — ')[0] || ''] ||
                          '#8e8e8e'}"></span>
                      <span
                        class="mt-1 ml-1 inline-block w-2 h-2 rounded-full {a.status === 'overdue'
                          ? 'bg-red-500'
                          : a.status === 'soon'
                            ? 'bg-yellow-500'
                            : 'bg-emerald-500'}"></span>
                      <div class="flex-1 min-w-0">
                        ...
                        <div class="mt-1 flex items-center gap-2">
                          ...
                          {#if a.subject}
                            <span
                              class="px-2 py-0.5 rounded-lg border text-xs"
                              style={chipStylesForCode(a.subject.split(' — ')[0] || a.subject)}
                              >{a.subject}</span>
                          {/if}
                        </div>
                      </div>
