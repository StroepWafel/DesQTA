<script lang="ts">
  ...
  async function loadForumComments(forumId: number) {
    ...
    try {
      ...
      logger.error('folios', 'loadForumComments', `Failed to load forum comments: ${e}`, {
        error: e,
      });
    } finally {
  ...
  const allComments = $derived(() => {
    ...
    const result: Array<
      (ForumMessage | ForumComment) & { isComment?: boolean; parentId?: number }
    > = [];
    ...
    <div
      ...
      <div class="flex items-center gap-4">
        ...
        <button
          ...
          {publishing
            ? $_('folios.publishing') || 'Publishing...'
            : published
              ? $_('folios.unpublish') || 'Unpublish'
              : $_('folios.publish') || 'Publish'}
        </button>
        ...
        <div class="max-w-6xl mx-auto p-6 space-y-6">
          ...
          <div
            ...
            <div class="flex flex-wrap gap-2 mb-3">
              {#each tags as tag}
                <span
                  class="inline-flex items-center gap-1 px-2 py-1 bg-accent-100 dark:bg-accent-900/30 text-accent-700 dark:text-accent-300 rounded-lg text-sm">
                  {tag}
                  <button
                    type="button"
          ...
          <div
            ...
            <div class="min-h-[600px] p-6">
              <Editor bind:content bind:editorInstance />
            </div>
          </div>
            ...
            <div class="space-y-4 mt-6">
              ...
              {#if loadingComments}
                <div class="flex justify-center items-center py-8">
                  <LoadingSpinner
                    size="sm"
                    message={$_('folios.loading_comments') || 'Loading comments...'} />
                </div>
