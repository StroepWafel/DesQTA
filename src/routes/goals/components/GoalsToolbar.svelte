<script lang="ts">
  ...
  const isBoldActive = $derived((editorUpdateTrigger >= 0 && editor?.isActive('bold')) || false);
  const isItalicActive = $derived((editorUpdateTrigger >= 0 && editor?.isActive('italic')) || false);
  const isUnderlineActive = $derived((editorUpdateTrigger >= 0 && editor?.isActive('underline')) || false);
  const isStrikeActive = $derived((editorUpdateTrigger >= 0 && editor?.isActive('strike')) || false);
  const isCodeActive = $derived((editorUpdateTrigger >= 0 && editor?.isActive('code')) || false);
...
<div class="tiptap-toolbar flex items-center justify-between p-2 bg-zinc-50 dark:bg-zinc-900/30">
          ...
          {#each blockTypes as blockType}
            ...
            {:else}
              <button
                ...
                onclick={() => setBlockType(blockType.id)}>
  ...
  <div class="flex items-center space-x-2">
    <button
      class="px-3 py-1.5 text-sm bg-accent-500 hover:bg-accent-600 text-white rounded-md transition-all duration-200 transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-1"
      onclick={onSave}
      disabled={saving || readonly || !editor}
      title="Save notes">
      {#if saving}
