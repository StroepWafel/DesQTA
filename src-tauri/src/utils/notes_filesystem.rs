...
pub fn save_note_filesystem(app: AppHandle, note: Note) -> Result<(), String> {
        ...
        .filter(|e| e.file_type().is_file() && e.path().extension().map_or(false, |ext| ext == "json"))
    {
        if let Ok(existing_fs_note) = load_note_file(entry.path()) {
            if existing_fs_note.id == note.id {
                let existing_filename = entry.path().file_stem()
                    .and_then(|s| s.to_str())
                    .unwrap_or("");
                ...
                if existing_filename != new_filename {
                    if let Err(e) = fs::remove_file(entry.path()) {
                        eprintln!("Failed to delete old note file {}: {}", entry.path().display(), e);
                    }
                }
                break;
            }
    ...
    let folder_path = if note.folder_path.is_empty() || note.folder_path[0] == "default" {
        ...
        if !path.exists() {
            fs::create_dir_all(&path).map_err(|e| format!("Failed to create folder structure: {}", e))?;
        }
        path
    };
...
#[tauri::command]
pub fn create_folder_filesystem(app: AppHandle, name: String, parent_path: Option<String>) -> Result<FileSystemFolder, String> {
    let notes_dir = get_notes_directory(&app)?;
...
pub fn delete_folder_filesystem(app: AppHandle, folder_path: String) -> Result<(), String> {
    ...
    let is_empty = full_path.read_dir()
...
#[tauri::command]
pub fn rename_folder_filesystem(app: AppHandle, old_path: String, new_name: String) -> Result<(), String> {
    let notes_dir = get_notes_directory(&app)?;
...
#[tauri::command]
pub fn move_note_filesystem(app: AppHandle, note_id: String, new_folder_path: Vec<String>) -> Result<(), String> {
    let notes_dir = get_notes_directory(&app)?;
...
fn load_note_file(path: &Path) -> Result<FileSystemNote, String> {
    ...
    serde_json::from_str(&contents)
        .map_err(|e| format!("Failed to parse note JSON: {}", e))
}
...
#[tauri::command]
pub fn get_image_as_base64_filesystem(_app: AppHandle, relative_path: String) -> Result<String, String> {
...
#[tauri::command]
pub fn restore_notes_from_backup_filesystem(app: AppHandle, backup_path: String) -> Result<(), String> {
    ...
    if backup_data.get("backup_type").and_then(|v| v.as_str()) == Some("filesystem") {
        ...
        let notes: Vec<Note> = serde_json::from_value(
            backup_data.get("notes").unwrap_or(&serde_json::json!([])).clone()
        ...
        return Err("This backup file is not compatible with the filesystem storage system".to_string());
    }
