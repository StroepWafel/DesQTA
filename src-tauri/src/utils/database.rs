    ...
    #[cfg(target_os = "android")]
    {
        ...
        if !dir.exists() {
            fs::create_dir_all(&dir)
                .context("Failed to create DesQTA data directory")?;
        }
...
pub fn db_cache_get(key: String) -> Result<Option<Value>, String> {
    ...
    let result: SqlResult<String> = stmt.query_row(
        params![key, now],
        |row| row.get(0)
    );
...
pub fn db_queue_all() -> Result<Vec<QueueItem>, String> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, type, payload, created_at FROM sync_queue ORDER BY created_at ASC"
    ).map_err(|e| e.to_string())?;
...
pub fn db_get_assessments_by_year(year: Option<i32>) -> Result<Vec<Value>, String> {
    ...
    if let Some(y) = year {
        let mut stmt = conn.prepare("SELECT data FROM assessments WHERE year = ? ORDER BY due DESC")
            .map_err(|e| e.to_string())?;
        let rows = stmt.query_map(params![y], |row| {
        ...
        let mut stmt = conn.prepare("SELECT data FROM assessments ORDER BY due DESC")
            .map_err(|e| e.to_string())?;
        let rows = stmt.query_map([], |row| {
            let data_str: String = row.get(0)?;
            serde_json::from_str::<Value>(&data_str)
